...
\subsection{Sliding Puzzle}
Completing the Sliding Puzzle minigame unlocks the Ukulele in the game's soundscape. This section will delve into the technical aspects of the sliding puzzle minigame. We'll begin by examining the teleportation mechanic, exploring how the player is seamlessly transferred from the snowy mountain environment to the dedicated puzzle platform. Following that, we'll deconstruct the individual components that make up the puzzle itself. This will include the invisible borders implemented to prevent the player from falling off the platform, ensuring a safe playing area. Finally, we'll analyse the scripts attached to these individual components, uncovering the logic and behaviours that govern their functionality within the puzzle.

\textbf{Teleportation Script} \newline
The sliding puzzle presents a unique intellectual challenge, requiring careful analysis and strategic thinking. To optimise the player's focus and mitigate potential distractions from the snowy mountain environment, they are briefly teleported to a separate platform. This isolated space minimises visual interference, allowing the player to fully concentrate on solving the puzzle. Upon completion, the player will be seamlessly returned to the mountain location to continue their journey.

The Teleportation script is responsible for managing the instantaneous movement of the player from the snowy environment to the sliding puzzle platform. The teleport target variable within the script holds a reference to the transform component of the destination platform, specifying the precise location where the player should be teleported. The OnTriggerEnter() function is automatically called when a collider enters the blue box's trigger zone. The function verifies if the entering object is indeed the player. Should that be the case, the script updates the player's position to the position of the teleport target.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.7\linewidth]{dissertation/images/teleportation_mechanism.png} 
...
The CubeMovement script is attached to each of the movable puzzle pieces within your sliding puzzle minigame. Its primary responsibility is managing the movement of these pieces and ensuring the fundamental rules of the puzzle are consistently upheld. The script includes an isMoving flag to prevent the player from attempting multiple moves simultaneously, to ensure smooth and predictable puzzle mechanics.

The OnTriggerEnter() function is automatically triggered upon collision between a puzzle piece's collider and another collider. This function checks if the colliding object is the player, ensuring that puzzle piece movement is exclusively initiated through direct player interaction. The CanMove() function plays a crucial role in determining the validity of a move based on the sliding puzzle's rules. It verifies that the puzzle piece is directly adjacent to the empty space and confirms the absence of any obstructing puzzle pieces blocking its path. Finally, the MoveWithLerp() coroutine is responsible for the visually seamless movement of the puzzle piece to the empty space's position. It leverages a function called 'Lerp' for linear interpolation, resulting in an smooth transition of the puzzle piece from its original space to the empty space. This coroutine also updates the isMoving flag to accurately manage the movement state of the puzzle piece.

\textbf{Ukulele Game Master Script} \newline
The Ukulele Game Master script serves as the central script for managing the sliding puzzle minigame. It is attached to an empty game object that functions as the parent of the sliding puzzle pieces. Its responsibility is to manage the logic, progress tracking, and win checking of the minigame. The script includes a cubes array that maintains references to all movable puzzle pieces. Additionally, the 'winPositions' array stores references to the designated target positions for each individual puzzle piece.  The player must correctly position all puzzle pieces in these target positions to successfully complete the puzzle.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.7\linewidth]{dissertation/images/sliding_puzzle_incomplete.png} 
  \caption{Screenshot of the sliding puzzle in progress. The player must strategically move the pieces to achieve the correct configuration.} 
  \label{fig:puzzle_incomplete} 
\end{figure}

The CheckWin() function determines these victory conditions. The function iterates through the cubes array, comparing the position of each puzzle piece to its corresponding target position stored within the 'winPositions' array. Should all pieces align with their designated targets, the function returns true, signifying the puzzle as complete. Conversely, if a mismatch exists, the function returns false, signifying the puzzle as incomplete.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.7\linewidth]{dissertation/images/sliding_puzzle_complete.png} 
